...
router.put("/assign", async (req, res) => {
  try {
    ...
    if (user.boothMember === true)
      return res
        .status(404)
        .send({ err: "This member is already a booth member" });
...
router.route("/application_form").post(async (req, res) => {
  try {
...
router.route("/application_form_view/:id").get(async (req, res) => {
  try {
...
router.route("/application_forms_view").get(async (req, res) => {
  try {
...
router.route("/application_form_update").put(async (req, res) => {
  try {
    let foundApplicant = await VGS_User.findOne({ email: req.body.email });
    if (!foundApplicant)
      return res
        .status(400)
        .send(`the applicant with this email is not found, check the email`);
    else {
      await VGS_User.update(
        { email: req.body.email },
        {
          ...
          appliedPosition:
            req.body.appliedPosition || foundApplicant.appliedPosition,
...
router.route("/deleteuser").delete(async (req, res) => {
  try {
    ...
    if (user.userType === "president")
      return res.status(404).send({ err: "you can not delete a president" });
    else {
...
router.route("/deletefromcommity").delete(async (req, res) => {
  try {
    ...
    if (user.userType === "Head")
      return res.status(404).send({ err: "you can not delete a head" });
    if (user.userType === "president")
      return res.status(404).send({ err: "you can not delete a president" });
    if (user.userType === "Advisor")
      return res.status(404).send({ err: "you can not delete an advisor" });
    if (user.userType === "Director")
      return res.status(404).send({ err: "you can not delete a president" });
    if (user.clubCommittee === null)
      return res.status(404).send({ err: "this member not in the commitee" });
...
//head can add member
router.route("/addmemberincommity").put(async (req, res) => {
  try {
    let user = await VGS_User.findOne({ email: req.body.email });
    if (!req.body.email)
      return res.status(400).send({ err: "email field is required" });
    if (!req.body.clubCommittee)
      return res.status(400).send({ err: "club committee field is required" });

    if (!user) return res.status(400).send({ err: "invalied user" });
    if (typeof req.body.clubCommittee != "string")
      return res.status(400).send({ err: "Invalid value for clubCommittee" });
    // check eno applicant
    if (user.userType === "president")
      return res.status(404).send({ err: "you can not add a president" });
    if (user.userType === "advisor")
      return res.status(404).send({ err: "you can not add an advisor" });
    if (user.userType === "director")
      return res.status(404).send({ err: "you can not add a president" });
    if (user.userType === "member")
      return res.status(404).send({ err: "you can not add a member" });
    if (user.userType === "head")
      return res.status(404).send({ err: "you can not add a head" });
...
router.route("/note").put(async (req, res) => {
  try {
    ...
    if (!req.body.email)
      return res.status(400).send({ err: "email field is required" });
    if (!req.body.notes)
      return res.status(400).send({ err: "note field is required" });
    if (!user) return res.status(400).send({ err: "invalid email" });
    if (typeof req.body.notes !== "string")
      return res.status(400).send({ err: "Invalid value for note" });
